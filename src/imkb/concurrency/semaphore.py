...
class AsyncSemaphore:
    ...
    def get_stats(self) -> SemaphoreStats:
        ...
        waiting_count = len(self._semaphore._waiters) if hasattr(self._semaphore, '_waiters') else 0
        
        # Calculate average hold time
        avg_hold_time = sum(self._hold_times) / len(self._hold_times) if self._hold_times else 0.0
...
class SemaphoreManager:
    ...
    def get_semaphore(self, name: str, capacity: Optional[int] = None) -> AsyncSemaphore:
        """
        ...
        try:
            ...
            for semaphore in sorted_semaphores:
                ...
                if timeout is not None and acquired:
                    elapsed = sum(
                        time.time() - s._active_acquisitions.get(0, time.time())
                        for s in acquired
                    )
...
def limit_concurrency(semaphore_name: str, capacity: Optional[int] = None, timeout: Optional[float] = None):
    """
    ...
    def decorator(func):
        ...
        if asyncio.iscoroutinefunction(func):
            ...
            logger.warning(f"Concurrency limiting not supported for sync function {func.__name__}")
            return sync_wrapper
