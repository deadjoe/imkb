...
class TracingConfig(BaseModel):
    ...
    def from_env(cls) -> "TracingConfig":
        ...
        return cls(
            ...
            otlp_headers=cls._parse_headers(os.getenv("OTEL_EXPORTER_OTLP_HEADERS", "")),
...
class MetricsConfig(BaseModel):
    ...
    collect_runtime_metrics: bool = Field(
        default=True,
        description="Collect Python runtime metrics"
    )
    collect_process_metrics: bool = Field(
        default=True, 
        description="Collect process metrics"
    )
    ...
    def from_env(cls) -> "MetricsConfig":
        ...
        return cls(
            ...
            collect_runtime_metrics=os.getenv("PROMETHEUS_RUNTIME_METRICS", "true").lower() == "true",
            collect_process_metrics=os.getenv("PROMETHEUS_PROCESS_METRICS", "true").lower() == "true",
        )
...
class LoggingConfig(BaseModel):
    ...
    include_span_id: bool = Field(
        default=True, 
        description="Include span ID in log records"
    )
...
class TelemetryConfig(BaseModel):
    ...
    def should_export_traces(self) -> bool:
        ...
        return (
            self.enabled and 
            self.tracing.enabled and 
            self.tracing.otlp_endpoint is not None
        )
