    ...
    async def generate(
        ...
        **kwargs
        ...
        try:
            ...
            metrics = get_metrics()
            if metrics:
                metrics.record_llm_request("openai", self.config.model, "default", kwargs.get("template_type", ""))
                if response.usage:
                    metrics.record_llm_tokens(
                        "openai", self.config.model, "default",
            ...
            metrics = get_metrics()
            if metrics:
                metrics.record_llm_error("openai", self.config.model, "default", type(e).__name__)
...
class LLMRouter:
    ...
    def _create_client(self, router_name: str, router_config: LLMRouterConfig) -> BaseLLMClient:
        """Create LLM client based on provider"""
        provider = router_config.provider.lower()
        
        if provider == "openai":
            # Check if API key is placeholder, use mock instead
            if router_config.api_key and router_config.api_key.startswith("sk-placeholder"):
                logger.info(f"Using mock client for placeholder API key in router {router_name}")
    ...
    async def health_check_all(self) -> Dict[str, bool]:
        ...
        results = {}
        for router_name in self.config.llm.routers.keys():
