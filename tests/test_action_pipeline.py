...
from imkb.action_pipeline import (
    ActionResult,
    ActionPipeline,
    gen_playbook
)
...
class TestActionResult:
    ...
    def test_basic_action_result(self):
        ...
        assert result.playbook == "1. Restart the service 2. Check error logs for issues"
...
class TestActionPipeline:
    ...
    async def test_search_similar_actions_error_handling(self):
        ...
        with patch.object(self.pipeline.mem0_adapter, 'search') as mock_search:
            ...
            similar_actions = await self.pipeline.search_similar_actions(self.rca_result)
...
This should resolve the issue."""
    ...
    def test_parse_action_response_invalid_json(self):
        ...
        llm_response = LLMResponse(
            content="This is not valid JSON response at all",
            model="gpt-4"
        )
    ...
    async def test_generate_actions_success(self):
        ...
        with patch.object(self.pipeline, 'search_similar_actions', return_value=mock_similar_actions):
            with patch.object(self.pipeline.llm_router, 'generate', return_value=mock_llm_response):
                with patch.object(self.pipeline, '_store_action_plan') as mock_store:
    ...
    async def test_generate_actions_error_handling(self):
        ...
        with patch.object(self.pipeline, 'search_similar_actions', side_effect=Exception("Search error")):
            result = await self.pipeline.generate_actions(self.rca_result)
    ...
    async def test_store_action_plan_error_handling(self):
        ...
        with patch.object(self.pipeline.mem0_adapter, 'add_memory', side_effect=Exception("Storage error")):
...
class TestActionPipelineIntegration:
    ...
    async def test_end_to_end_action_workflow(self):
        ...
        rca_data = {
            ...
            "extractor": "mysqlkb",
            "references": [
                {"excerpt": "Connection pool issues are common", "source": "mysql_kb", "confidence": 0.9, "metadata": {}}
            ],
            "immediate_actions": ["Increase max_connections", "Monitor pool usage"],
            "preventive_measures": ["Add connection monitoring", "Review pool configuration"],
