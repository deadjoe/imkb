...
from imkb.cli import cli, get_rca, gen_playbook, config
    ...
    def test_get_rca_error_handling(self, mock_get_rca):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            ...
            try:
                ...
                assert result.exit_code == 0  # CLI doesn't exit with error, just prints error
...
class TestGenPlaybookCommand:
    ...
    def setup_method(self):
        ...
        self.rca_data = {
            ...
            "confidence": 0.85,
            "extractor": "mysqlkb",
            "references": [{"excerpt": "Pool issue", "source": "mysql_kb", "confidence": 0.9, "metadata": {}}],
    ...
    def test_gen_playbook_success(self, mock_gen_playbook):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            ...
            try:
                ...
                assert call_args[0][0]["root_cause"] == "MySQL connection pool exhausted"
                assert call_args[0][1] == "default"  # default namespace
            ...
            try:
                ...
                with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as rca_file:
                    ...
                    try:
                        ...
                        playbook_result = self.runner.invoke(gen_playbook, ['--rca-file', rca_file.name])
