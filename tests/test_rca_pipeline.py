...
class TestRCAResult:
    ...
    def test_rca_result_from_dict(self):
        ...
        data = {
            ...
            "confidence": 0.9,
            "extractor": "network",
            "references": [{"doc_id": "kb_004", "excerpt": "Timeout info", "score": 0.8, "metadata": {}}],
            "status": "SUCCESS",
...
class TestRCAPipeline:
    ...
    async def test_find_no_matching_extractor(self):
        ...
        with patch.object(self.pipeline, 'get_extractors', return_value=[mock_extractor]):
            event = Event(id="test", signature="test.event", timestamp="2024-01-01T12:00:00Z", severity="info", source="test", labels={}, message="Test")
            extractor = await self.pipeline.find_matching_extractor(event)
...
This analysis indicates a clear memory issue."""
    ...
    def test_parse_llm_response_invalid_json(self):
        ...
        assert isinstance(result, RCAResult)
        assert "parse" in result.root_cause.lower() or "failed" in result.root_cause.lower()
    ...
    async def test_generate_rca_success(self):
        ...
        event = Event(id="test", signature="test.event", timestamp="2024-01-01T12:00:00Z", severity="info", source="test", labels={}, message="Test description")
        ...
        mock_llm_response = LLMResponse(
            content='{"root_cause": "Test cause", "confidence": 0.8}',
            model="gpt-4"
        )
...
class TestGetRCAFunction:
    ...
    async def test_get_rca_no_extractor(self, mock_pipeline_class):
        ...
        event_data = {"id": "test", "signature": "test.event", "timestamp": "2024-01-01T12:00:00Z", "severity": "info", "source": "test", "labels": {}, "message": "Test"}
...
class TestRCAPipelineIntegration:
    ...
    async def test_namespace_isolation(self):
        ...
        # Both should succeed
        assert result1["status"] in ["SUCCESS", "PARSE_ERROR"]  # Mock LLM might have parse issues
